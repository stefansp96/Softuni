...
def swap_lessons(list_of_lessons, first_title, second_title):
    if first_title in list_of_lessons and second_title in list_of_lessons:
        first_lesson_index = list_of_lessons.index(first_title)
        second_lesson_index = list_of_lessons.index(second_title)
        first_has_exercise = False
        second_has_exercise = False

        if f"{first_title}-Exercise" in list_of_lessons:
            first_has_exercise = True
        if f"{second_title}-Exercise" in list_of_lessons:
            second_has_exercise = True

        list_of_lessons[first_lesson_index], list_of_lessons[second_lesson_index] = \
            list_of_lessons[second_lesson_index], list_of_lessons[first_lesson_index]

        if first_has_exercise and second_has_exercise:
            list_of_lessons[first_lesson_index + 1], list_of_lessons[second_lesson_index + 1] = \
                list_of_lessons[second_lesson_index + 1], list_of_lessons[first_lesson_index + 1]
        elif not first_has_exercise and second_has_exercise:
            list_of_lessons.insert(first_lesson_index + 1, list_of_lessons.pop(second_lesson_index + 1))
        elif first_has_exercise and not second_has_exercise:
            list_of_lessons.insert(second_lesson_index + 1, list_of_lessons.pop(first_lesson_index + 1))
...
def exercise(list_of_lessons, title):
    if title in list_of_lessons and f"{title}-Exercise" not in list_of_lessons:
        ...
        list_of_lessons.inser(lesson_index + 1, f"{title}-Exercise")
...
while command != 'course start':
    ...
    if action == 'Add':
        schedule = add_lesson(schedule, lesson_title)
    elif action == 'Insert':
        schedule = insert_lesson(schedule, lesson_title, int(index_or_title))
    elif action == 'Remove':
        schedule = remove_lesson(schedule, lesson_title)
    elif action == 'Swap':
        schedule = swap_lessons(schedule, lesson_title, index_or_title)
